...
static int vexriscv_parse_cpu_file(struct command_context *cmd_ctx, struct target *target){
	...
	if(!input){
		...
		//goto error;
		LOG_DEBUG("cpuConfigFile %s not found", vexriscv->cpuConfigFile);
    return ERROR_OK;
	}
...
COMMAND_HANDLER(vexriscv_handle_breakpointCount_command)
{
	if (CMD_ARGC != 1)
		return ERROR_COMMAND_ARGUMENT_INVALID;
	struct target* target = get_current_target(CMD_CTX);
	struct vexriscv_common *vexriscv = target_to_vexriscv(target);
	COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], vexriscv->hardwareBreakpointsCount);
	return ERROR_OK;
}
...
static const struct command_registration vexriscv_exec_command_handlers[] = {
   {
    ...
    .name = "breakpointCount",
		.handler = vexriscv_handle_breakpointCount_command,
		.mode = COMMAND_CONFIG,
		.help = "Hardware breakpoints count that implemented by target.",
		.usage = "value"
	},
